...
if COMPLETE:
    for t, des in uni_vocab.transform(pca_t.transform(feature_extractor.extract(voc2006, 'train')), force=True):
        print t['img_id']
    for t, des in uni_vocab.transform(pca_t.transform(feature_extractor.extract(voc2006, 'test')), force=True):
        print t['img_id']

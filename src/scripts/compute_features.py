...
@click.option('--dataset', default='regular')
@click.option('--storage-name', default='')
def main(sname, iteration, cropped, flipped, force, dataset, storage_name):
    ...
    if dataset == 'segmented':
        ...
        cub = CUB_200_2011_Parts_Body(settings.CUB_ROOT)
        ...
        cub = CUB_200_2011(settings.CUB_ROOT, images_folder_name=dataset)
    if not storage_name:
        ft_storage = datastore(settings.storage(new_name))
    else:
        ft_storage = datastore(settings.storage(storage_name))
    ft_extractor = CNN_Features_CAFFE_REFERENCE(ft_storage, model_file=settings.model(new_name), pretrained_file=settings.pretrained(new_name), crop_index=0)

...
class VOC2006AnnotationParser(object):
    ...
    def get_objects(self, trunc=True, difficult=False):
        objects = []
        for match in re.finditer(self.RE_OBJECT_DEF,
                                 self.annotation_file_contet):
            obj_index, obj_label, original_obj_label = match.groups()
            obj_index = int(obj_index)
            xmin, ymin, xmax, ymax = re.search(
                self.RE_OBJECT_BB % (obj_index, obj_label),
                self.annotation_file_contet).groups()
            ...
            if not difficult and self.DIFFICULT in original_obj_label:
                continue
            objects.append({'ind': obj_index, 'label': obj_label,
                            'original_label': original_obj_label,
                            'xmin': xmin, 'ymin': ymin, 'xmax': xmax,
                            'ymax': ymax,
                            'trunc': self.TRUNC in original_obj_label,
                            'difficult': self.DIFFICULT in original_obj_label,
                            'class': which_one(original_obj_label,
                                               self.CLASSES),
                            'view': which_one(original_obj_label, self.VIEWS)})
